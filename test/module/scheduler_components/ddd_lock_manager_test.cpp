...
class DDDLockManagerTest : public ::testing::Test {
protected:
  DDDLockManager lock_manager;
...
TEST_F(DDDLockManagerTest, GetAllLocksOnFirstTry) {
...
TEST_F(DDDLockManagerTest, WriteLocks) {
  auto configs = MakeTestConfigurations("locking", 1, 1);
...
TEST_F(DDDLockManagerTest, PartiallyAcquiredLocks) {
  auto configs = MakeTestConfigurations("locking", 1, 1);
  auto txn1 = FillMetadata(MakeTransaction({"A"}, {"B", "C"}));
  txn1->mutable_internal()->set_id(100);
  txn1->mutable_internal()->set_num_lock_only_txns(1);
...
TEST_F(DDDLockManagerTest, PrioritizeWriteLock) {
  auto configs = MakeTestConfigurations("locking", 1, 1);
...
TEST_F(DDDLockManagerTest, AcquireLocksWithLockOnlyTxn1) {
  auto configs = MakeTestConfigurations("locking", 1, 1);
  auto txn1 = FillMetadata(MakeTransaction({"A"}, {"B", "C"}));
  txn1->mutable_internal()->set_id(100);
  txn1->mutable_internal()->set_num_lock_only_txns(1);
  ...
  auto txn2 = FillMetadata(MakeTransaction({"A"}, {"B"}));
  txn2->mutable_internal()->set_id(200);
  txn2->mutable_internal()->set_num_lock_only_txns(2);
  TransactionHolder holder2 = MakeHolder(configs[0], txn2);
...
TEST_F(DDDLockManagerTest, AcquireLocksWithLockOnlyTxn2) {
  auto configs = MakeTestConfigurations("locking", 1, 1);
  auto txn1 = FillMetadata(MakeTransaction({"A"}, {"B", "C"}));
  txn1->mutable_internal()->set_id(100);
  txn1->mutable_internal()->set_num_lock_only_txns(1);
...
TEST_F(DDDLockManagerTest, AcquireLocksWithLockOnlyTxnOutOfOrder) {
  auto configs = MakeTestConfigurations("locking", 1, 1);
  auto txn1 = FillMetadata(MakeTransaction({"A"}, {"B", "C"}));
  txn1->mutable_internal()->set_id(100);
  txn1->mutable_internal()->set_num_lock_only_txns(1);
...
TEST_F(DDDLockManagerTest, MultiEdgeBetweenTwoTxns) {
  auto configs = MakeTestConfigurations("locking", 1, 1);
...
TEST_F(DDDLockManagerTest, KeyReplicaLocks) {
  auto configs = MakeTestConfigurations("locking", 1, 1);
  auto txn1 = FillMetadata(MakeTransaction({}, {"writeA", "writeB"}));
  txn1->mutable_internal()->set_id(100);
  txn1->mutable_internal()->set_num_lock_only_txns(1);
...
TEST_F(DDDLockManagerTest, RemasterTxn) {
  ...
  txn->mutable_internal()->set_num_lock_only_txns(2);
  TransactionHolder holder = MakeHolder(configs[0], txn);
...
TEST_F(DDDLockManagerTest, LongChain) {
  auto configs = MakeTestConfigurations("locking", 1, 1);
