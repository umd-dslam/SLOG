...
TEST(CommandsTest, SimpleKeyValueProcedures) {
  auto txn = MakeTransaction({"key1"}, {"key2", "key3", "key4"},
                             "GET    key1   \n"
                             "SET   key2   value2  \n"
                             "DEL    key4   \n"
                             "COPY    key1 key3\n");
...
TEST(CommandsTest, KeyValueAbortedNotEnoughArgs) {
  auto txn = MakeTransaction({}, {"key1", "key2", "key3"}, "SET key1");
...
TEST(CommandsTest, KeyValueAbortedInvalidCommand) {
  auto txn = MakeTransaction({}, {"key1", "key2", "key3"}, "WRONG");
...
TEST(CommandsTest, KeyValueOnlyWritesKeysInWriteSet) {
  auto txn = MakeTransaction({"key1"}, {"key2", "key3"},
                             "GET key1\n"
                             "SET key2 value2\n"
                             "SET key4 value4\n"
                             "DEL key3");
